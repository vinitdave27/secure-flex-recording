...
const REACT_APP_RECORD_CHANNEL = process.env.REACT_APP_RECORD_CHANNEL.toLowerCase();
...
const addCallDataToTask = async (task, callSid, recording) => {
	...
	if (recording) {
		...
		const mediaObj = {
			//url: recordingUrl,
			//NOTE: We have commented the url attribute/property below, as we will set the url or url_provider
			// attribute/property
			url_provider: recordingUrl,
			type: 'VoiceRecording',
...
const handleAcceptedCall = async (task) => {
	...
	if (conversations && conversations.media && REACT_APP_RECORD_CHANNEL == 'customer') {
	...
	const participantLeg = participants.find((p) => p.participantType === REACT_APP_RECORD_CHANNEL);
